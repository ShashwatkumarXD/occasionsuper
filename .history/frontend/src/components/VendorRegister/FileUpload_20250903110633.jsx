...
const FileUpload = ({ label, description, onFileSelect, required = false, acceptedTypes = "*" }) => {
  ...
  return (
    <div className="outline-2 outline-[#E69B83] border-gray-300 rounded-lg p-3 mb-6 bg-white">
      ...
      {uploadedUrls.length > 0 && (
        <div className="mt-3">
          <h4 className="font-medium text-sm text-green-700 mb-2">Uploaded Successfully:</h4>
          <ul className="space-y-1">
            {uploadedUrls.map((url, idx) => (
              <li key={idx} className="text-xs text-green-600 break-all">
                ✅ {url}
              </li>
            ))}
          </ul>
