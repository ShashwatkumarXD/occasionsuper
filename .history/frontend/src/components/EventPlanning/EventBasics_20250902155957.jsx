...
const EventBasics = ({ formData, setFormData }) => {
  ...
  //         `http://localhost:3000/api/cities?q=${encodeURIComponent(inputValue)}`,
  ...
  // Fetch cities dynamically from OpenStreetMap Nominatim API
  // useEffect(() => {
  //   if (inputValue.length < 2) return; // wait for 2+ chars
  //   const controller = new AbortController();

  //   fetch(
  //     `http://localhost:3000/api/cities?q=${inputValue}`,
  //     {
  //       signal: controller.signal,
  //       headers: {
  //         "User-Agent": "EventPlannerApp", // Nominatim requires this
  //       },
  //     }
  //   )
  //     .then((res) => res.json())
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/cities?q=${encodeURIComponent(inputValue)}`, {
      signal: controller.signal,
    })
      .then((res) => {
        if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
