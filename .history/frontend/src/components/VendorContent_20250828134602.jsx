...
/**
 * Vendor Dashboard – single-file React app
 * TailwindCSS required. Icons from lucide-react.
 * Drop this file into your Vite React project and render <VendorDashboard />
 */
...
const Section = ({ title, right, children }) => (
  <section className="bg-white/60 backdrop-blur rounded-2xl p-6 shadow-sm border border-black/[0.03]">
...
const StatCard = ({ icon, title, value, bg = "bg-[#fff5f2]" }) => (
  <div className={`${bg} rounded-2xl p-4 shadow-sm flex items-center`}>
    <div className="basis-2/5 flex justify-center">{icon}</div>
...
const TopNav = ({ tab, setTab }) => (
  <nav className="bg-white rounded-2xl p-2 shadow-sm border border-black/[0.04]">
    <ul className="flex flex-wrap items-center justify-between gap-2 md:gap-3">
      {Tabs.map((t) => (
        <li key={t} className="grow">
          <button
            onClick={() => setTab(t)}
            className={`w-full px-6 py-2 rounded-full font-medium transition text-center ${
              tab === t
                ? "bg-[#e39a79] text-white shadow-sm"
...
const Overview = () => (
  <div className="space-y-6">
    ...
    <div className="flex items-start justify-between">
      <div>
        <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-100 text-orange-600 text-sm font-medium">
          <Crown className="w-4 h-4" /> Verified Vendor
        </span>
      ...
      <div className="text-right">
        ...
        <p className="text-gray-500 text-sm mt-2">Member since 8/26/2025</p>
      </div>
    </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      />
      <StatCard
        icon={<CalendarIcon className="w-10 h-10 text-teal-500" />}
        title="Upcoming Jobs"
        value="0"
        bg="bg-[#f0f6f8]"
      />
    ...
    {/* Monthly performance & Service breakdown */}
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {[
            ...
            <div key={m} className="flex items-center gap-4">
              ...
              <div className="flex-1 h-3 bg-gray-200 rounded-full overflow-hidden">
                <div className="h-3 bg-[#e39a79] rounded-full w-2/3"></div>
              </div>
      ...
      <Section title="Service Breakdown">
        <div className="space-y-5">
          {[
            { name: "Wedding Photography", jobs: 12, pct: 0.85 },
            { name: "Corporate Events", jobs: 8, pct: 0.65 },
            { name: "Birthday Parties", jobs: 15, pct: 0.95 },
          ].map((s) => (
            <div key={s.name}>
        ...
        {[
          ...
          >
            <div className="flex items-center gap-3 text-gray-700">
              ...
              <div>
                <p className="font-semibold">Event on {a.date}</p>
...
const LeadsPage = () => (
  <div className="space-y-6">
    <div className="flex items-center justify-between">
      ...
      <Badge tone="yellow">1 New Leads</Badge>
    </div>
...
const MonthCalendar = () => {
  ...
  startDay.setDate(1 - ((firstDay.getDay() + 6) % 7)); // start from Sunday-like grid
...
export default function VendorContent() {
  ...
  return (
    <div className="min-h-screen border-2 bg-[#f7f8fc] p-4 md:p-8">
      ...
      {/* Sticky bottom bar like mobile helpers */}
      <div className="fixed bottom-4 inset-x-0 flex justify-center pointer-events-none">
        <div className="pointer-events-auto flex items-center gap-3 bg-white/90 backdrop-blur px-3 py-2 rounded-full shadow border border-black/[0.04]">
