...
const Register = () => {
    ...
    const isStepValid = (currentStep) => {
        if (currentStep === 1) {
            return (
                ...
                formData.ownerName &&
                formData.email &&
                formData.phone && /^(\d){7,15}$/.test(String(formData.phone)) &&
                formData.city &&
    ...
    const handleSubmit = async () => {
        if (
            ...
            !formData.email ||
            !formData.phone ||
            !/^(\d){7,15}$/.test(String(formData.phone)) ||
            !formData.city ||
            !formData.serviceArea ||
            !formData.socialMedia ||
            !formData.categories.length
        ) {
            toast.error("Please fill in all required fields (valid phone and social link)");
            return;
        }
